...
//	gl.uniform1i(u_Kshiny, 4);						// Kshiny shinyness exponent
  ...
  var tick = function() {
      ...
      modelMatrix.setRotate(90+num, 0, 1, 0); // Rotate around the y-axis
      // Calculate the view projection matrix
      mvpMatrix.setPerspective(30, canvas.width/canvas.height, 1, 100);
      mvpMatrix.lookAt( 2, 5, 6,        // eye
                    0, 0, 0,        // aim-point
                    0, 1, 0);       // up.
...
function getGroundGrid() {
  ...
  var ycount = 100; 
  var xymax = 50.0;
  var color = new Float32Array([0.5, 0.5, 0.5]);
  
  gndVerts = new Float32Array(6*2*(xcount+ycount));
  ...
  for(v=0, j=0; v<2*xcount; v++, j+= 6) {
    ...
    gndVerts[j+3] = color[0];
    gndVerts[j+4] = color[1];
    gndVerts[j+5] = color[2];
  }
